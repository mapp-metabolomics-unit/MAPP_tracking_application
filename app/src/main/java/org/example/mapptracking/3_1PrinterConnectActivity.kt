// 3.1)
// Page that asks the user if he wants to connect a brady printer or not.
// If yes, redirected to ManagePrinterActivity, else directly to HomePageActivity
...
class `3_1PrinterConnectActivity` : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        connectButton.setOnClickListener {
            val intent = Intent(this,`3_2ManagePrinterActivity`::class.java)
        ...
        ignoreButton.setOnClickListener {
            val intent = Intent(this, `4HomePageActivity`::class.java)
            intent.putExtra("ACCESS_TOKEN", accessToken)
            intent.putExtra("USERNAME", username)
            intent.putExtra("PASSWORD", password)
            intent.putExtra("IS_PRINTER_CONNECTED", isPrinterConnected)
